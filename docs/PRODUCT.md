...
Track management is now symmetric between Map and Tracks: both offer multi-select, and Merge is a named first-class workflow with replace-by-default semantics plus preserved-gap, direct, and bicycle-routed connectors. Every track belongs to exactly one project group. Imports receive an explicit current/new-project decision, one source group per file, and exact duplicate suppression for tracks, routes, and waypoints. Groups contain their track rows directly, can be collapsed or hidden as a unit, and selected tracks can move into an existing or new group; moving all tracks from one group into another naturally merges them. Hidden groups are excluded from map picking and lasso selection while retaining member-level visibility preferences. Track-list scroll and focus behavior are project state, and large-file chart/handle rendering has bounded presentation work without reducing editing or export precision. Entering Tracks reveals and scrolls to the primary map selection, expanding only a group that would otherwise hide it.

The 1.4.3 large-project pass streams GPX parsing and batches MapLibre track geometry. A real 10.7 MB tour containing 157 tracks and 122,773 total points opens in about three seconds on the reporting Android device; cancellation remains interruptible, and full editing/export precision is unchanged.
